...
//    }
    if (empty($error)){
        if (!empty($name_avatar)){
            ...
            $name_avatar = $data['avatar'];
        }
        $sql = "UPDATE users SET fullname='".$fullname."',email='".$email."',avatar='".$name_avatar."',phone='".$phone."',status='".$status."',updated_at='".$update_at."'";
        print_r($sql);
        $result = mysqli_query($db,$sql);
        if($result){
            $success['success_edit'] = "Sửa thành công";
            header('Location: ' . $_SERVER['HTTP_REFERER']);
        }else{
            $error['fail_edit'] = "Sửa thất bại";
        }
    }
}
?>
    ...
    <form action="" method="post" enctype="multipart/form-data">
        <div class="col-md-8">
            <div class="white-box">
                ...
                <?php if (isset($success['success_edit'])){?>
                    <div class="my-alert">
                        <div class="alert alert-success" style="height: 0;">
                            <p style="color:#0d4e02;text-align: center;line-height: 0;"><?=$success['success_edit']?></p>
                            <button type="button" class="close" data-dismiss="alert" aria-label="Close" style="line-height: 0;">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                    </div>
                <?php }elseif (isset($error['fail_edit'])){?>
                    <div class="my-alert">
                        <div class="alert alert-danger" style="height: 0;">
                            <p style="color:#882406;text-align: center;line-height: 0;"><?=$error['fail_edit']?></p>
                            <button type="button" class="close" data-dismiss="alert" aria-label="Close" style="line-height: 0;">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
